$(function(){
    ...
    function setting(adjust) {
        ...
        $proList.eq(selNum).css({ left : adjust1 * 200 + 'px', display : 'block', opacity :0,position:'relative' })
        $numList.eq(selNum).css({color:'#25aae1'})
        $numList.eq(oldNum).css({color:'gray'})
        $proList.eq(oldNum).css({display:'none'})
